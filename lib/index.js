...
module.exports = function (opts) {
  ...
  return async function (next) {
    ...
    const { mod, findCommand, exec, oneport, mkdirp, readFile, writeFile, prompt } = this.utils;
    ...
    const commandPath = findCommand(this.cwd, command);
    ...
    if ( command === 'start-storybook' ){
      ...
      if ( !fs.existsSync(assetsDir) ){
        script = `${commandPath} -h ${host} -p ${port} -c ${config}`;
      } else {
        script = `${commandPath} -h ${host} -p ${port} -s ${assets} -c ${config}`;
      };
      ...
      // 拼接命令
      if ( !fs.existsSync(assetsDir) ){
        script = `${commandPath} -c ${config} -o ${output}`;
      } else {
        script = `${commandPath} -s ${assets} -c ${config} -o ${output}`;
      };
